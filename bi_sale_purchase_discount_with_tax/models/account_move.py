...
#                 name = line.name
#             if rslt[0].discount_type == 'line':
#                 price = rslt[0].discount_amt_line

#             elif rslt[0].discount_type == 'global':
#                 price = rslt[0].discount_amt
#             else:
#                 price = 0

                if (res.move_type == "out_invoice") or (res.move_type == "out_receipt") or (res.move_type == "in_refund"):
                    if rslt[0].discount_type == 'line':
                        price = rslt[0].discount_amt_line
                    elif rslt[0].discount_type == 'global':
                        price = rslt[0].discount_amt
                ...
                for line in rslt.line_ids:
                    ...
                    if (res.move_type == "in_invoice") or (res.move_type == "in_receipt") or (res.move_type == "out_refund"):
            ...
            if rslt.line_ids:
                if name != 'Discount':
                    if rslt.discount_account_id:
                        discount_vals = {
                            'account_id': rslt.discount_account_id[0],
                            'quantity': 1,
                            'price_unit': -price,
                            'name': "Discount",
                            'exclude_from_invoice_tab': True,
